...
import { upsertCulture } from "@/server/actions/admin/culture";

interface CultureEditPageProps {
  params: Promise<{ slug: string; id?: string }>;
}

export default async function CultureEditPage({ params }: CultureEditPageProps) {
  const { slug, id } = await params;
  const country = await db.query.countries.findFirst({ where: eq(countries.slug, slug) });
  if (!country) return null;

  let item = null;
  if (id) item = await db.query.cultureItems.findFirst({ where: eq(cultureItems.id, id) });
    ...
    <div>
      ...
      <form action={upsertCulture} className="mt-6 space-y-4">
        ...
        <div className="grid gap-4 sm:grid-cols-2">
          ...
          <div><label className="block font-mono text-[10px] uppercase tracking-[0.15em] text-mist mb-1.5">Title *</label><input name="title" defaultValue={item?.title ?? ""} required className="w-full rounded border border-white/15 bg-ink px-3 py-2 font-mono text-sm text-paper focus:border-accent focus:outline-none" /></div>
        </div>

        <div>
          <label className="block font-mono text-[10px] uppercase tracking-[0.15em] text-mist mb-1.5">Category *</label>
          <input name="category" defaultValue={item?.category ?? ""} required className="w-full rounded border border-white/15 bg-ink px-3 py-2 font-mono text-sm text-paper focus:border-accent focus:outline-none" />
        </div>
        ...
        <a href={`/admin/countries/${slug}/culture`} className="ml-4 font-mono text-[11px] uppercase tracking-[0.15em] text-mist hover:text-paper">Cancel</a>
      </form>
    </div>
  );
}
